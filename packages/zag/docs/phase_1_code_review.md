...
> Review of the zag MVP (Lexer → Reader → Emitter → CLI) after getting the first `.zag` files compiling to native binaries and WASM.
...
```
.zag ──► Lexer ──► Reader ──► Form AST ──► Emitter ──► .zig ──► zig build ──► binary/wasm
```
...
A single integration test that compiles a `.zag` file and asserts on its stdout would catch half the bugs in this review.
...
1. **Add integration tests** — Compile `.zag` → `.zig` → binary, run it, assert stdout.
...
12. **Self-hosted core library** — `lib/core.zag` is still empty.

---

## 7. Summary

The MVP achieves its goal: **zag compiles to working Zig, which compiles to native and WASM**. The architecture is sound. The main issues are:
